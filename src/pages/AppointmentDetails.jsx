  ...
  return (
    <div className="min-h-screen bg-slate-50">
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <h1 className="text-2xl font-bold text-emerald-600 mb-4">Appointment details</h1>
        ...
        <h2 className="text-lg font-bold text-gray-900 mb-4">Appointment</h2>
           
        <div className="space-y-3 text-sm text-gray-700">
              <p>
                <span className="font-semibold">Date:</span> {appointment?.queueEntry?.queueDate || "—"}{" "}
                <span className="mx-1">·</span>{" "}
                <span className="font-semibold">Time:</span> {appointment?.timeSlot || "—"}
              </p>
             
              <p className="flex items-center">
                <span className="font-semibold mr-1">Token:</span> {appointment?.queueEntry?.tokenNumber || "—"}{" "}
                <span className="mx-2">·</span>{" "}
                
                <span className="font-semibold mr-2">Status:</span>
               
                <span className="inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium bg-yellow-100 text-yellow-800 capitalize">
                  {appointment?.queueEntry?.status || "waiting"}
                </span>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
            <h2 className="text-lg font-bold text-gray-900 mb-4">Medicines (Prescription)</h2>

            {appointment?.prescription ? (
              <p className="text-sm text-gray-600">{appointment.prescription}</p>
            ) 
            : (
              <p className="text-sm text-gray-500">No prescription added for this appointment yet.</p>
            )}
          </div>
